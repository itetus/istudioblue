...
class IstudioblueSupabaseUser extends BaseAuthUser {
  IstudioblueSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> istudioblueSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IstudioblueSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
